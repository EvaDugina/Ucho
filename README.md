...
Закрытый Telegram-бот, говорящий от лица Иуды Искариота. Бот задаёт вопросы, сохраняет
...
- автоматический вопрос через случайные 4–7 календарных дней и характерная
  реплика Иуды за день до него, если предыдущий вопрос остался без ответа;
...
- персональные книжные toggles и scores; автоматические цитаты временно выключены;
...
Автоматический вопрос проверяется ежедневно в `DAILY_HOUR`. После каждой отправки
бот один раз выбирает интервал 4–7 дней и сохраняет дату следующего вопроса. Если
ответа нет, за день до этой даты приходит одна контекстная реплика Иуды.
`BOOK_REMINDERS_ENABLED=false` отдельно выключает автоматические книжные цитаты,
не затрагивая `/upload`, `/sea` и ручные книжные разговоры.
...
ровно на один. Автоматическая доставка новых цитат временно выключена; настройки и
обработка ранее отправленной цитаты сохранены.
...
- В вопросах, реакциях и `/about` бот говорит голосом Иуды, обращаясь к
  собеседнику на «ты», без титулов. Художественная биография не приписывается пользователю.
