...
Разделение труда не меняется: лексикон даёт сырой VAD-якорь, арбитр (Qwen) решает.
...
- Файла нет / pymorphy3 недоступен / нет совпадений слов → `score` отдаёт None
  (мягкая деградация: пайплайн настроения работает на одном Qwen, ответ не падает).
