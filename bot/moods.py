...
- **LLM** только классифицирует последнее сообщение категориально
  (`sign/energy/direction/quality`) — `llm.classify_mood`.
...
def to_numeric(per_msg: dict) -> tuple[int, int]:
    """Категории сообщения → (valence, arousal) ∈ {-1,0,1}."""
    p = normalize_per_msg(per_msg)
    return _sign_num(p["sign"]), _energy_num(p["energy"])
...
def session_mood(trajectory: list[dict], prior: tuple[float, float] = (0.0, 0.0)) -> dict:
    ...
    (valence, arousal) из портрета (`about.mood_baseline`); доминирует на старте
    сессии, затухает по мере накопления сообщений.

    Returns: {valence, arousal ∈[-1..1]; sign, energy — дискретно; quality;
    direction; stability ∈ {labile,adequate,rigid}; n}.
    """
    traj = [t for t in (trajectory or []) if isinstance(t, dict)]
    n = len(traj)
    pv, pa = float(prior[0]), float(prior[1])
    if n == 0:
        return {
            "valence": pv, "arousal": pa,
...
def mood_label(mv: dict) -> str:
    """Компактная строка настроения для портрета: «грусть_тоска − low rigid»."""
    return f"{mv.get('quality','спокойствие')} {mv.get('sign','0')} {mv.get('energy','normal')} {mv.get('stability','adequate')}"
...
_vader = None  # ленивый singleton SentimentIntensityAnalyzer


def vader_compound(en_text: str):
    """Тональность англ. текста через VADER → {neg,neu,pos,compound} или None.

    compound ∈ [-1..1] — инструментальная оценка валентности (подсказка арбитру-LLM).
    Сбой/пустой текст → None (VADER пропускается). Текст уже переведён на англ.
    (`translate.translate_ru_en`) — VADER работает только по-английски.
    """
    en = (en_text or "").strip()
    if not en:
        return None
    ...
    try:
        ...
        return _vader.polarity_scores(en)
    except Exception:
        log.exception("vader_compound failed (non-fatal)")
        return None
...
def log_turn(mood_vec: dict, bot_mood: str, vader: dict | None = None) -> None:
    """Записать пару (настроение → лицо) в `_mood_log.jsonl` (кольцо). Для графа
    Фазы D (weekly-review агрегирует в `mood/` + `_mood_map.json`). `vader_compound`
    — сверка арбитр↔лексикон (где LLM перебивает VADER)."""
    try:
        ...
        entry = {
            ...
            "vader_compound": (vader or {}).get("compound") if isinstance(vader, dict) else None,
        }
        p = _mood_log_path()
