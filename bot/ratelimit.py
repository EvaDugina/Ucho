"""Per-user ограничитель LLM-операций (anti-DoS на внешний LLM-контур).

Контекст: live-запросы идут во внешний OpenAI-compatible provider. Без ограничителя
один гость спамом вопросов мог бы непрерывно занимать лимиты/слоты модели и
блокировать остальных (в т.ч. владельца). Здесь — два барьера на пользователя:

1. **single-flight** — не более одного активного LLM-вызова на пользователя.
   Пока предыдущий обрабатывается, команды получают busy-ответ; обычный текст
   в probe-сессии выше по слою складывается в durable `queued_answer`.
...
def is_inflight(uid: Optional[int]) -> bool:
    """True, если у пользователя прямо сейчас идёт LLM-операция."""
    return uid is not None and uid in _inflight
