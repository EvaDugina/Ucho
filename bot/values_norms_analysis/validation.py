"""Валидация кандидатов анализа 03 до записи в граф."""
from __future__ import annotations

import re

from .models import ValuesNormsCandidate
...
def _type_for(category: str, value: object = None) -> str:
    del value
    return {
        "values": "value",
        "ideals": "ideal",
...
) -> tuple[list[ValuesNormsCandidate], int]:
    """Вернуть валидные кандидаты и число отброшенных элементов."""
    if not isinstance(raw_candidates, list):
        return [], 0
    out: list[ValuesNormsCandidate] = []
    dropped = 0
