...
1. `reaction` — короткая реплика Иуды от первого лица, 1–3 предложения, не вопрос.
...
3. `mask_frequency_draft` — необязательный частичный объект `{лицо: 0..1}`.
...
```json
{
  ...
  "mask_frequency_draft": {"сомнение": 0.1}
}
```
