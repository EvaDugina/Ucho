...
- `bot/about.py` — портрет пользователя (`about_user.md` + `_user_deltas.jsonl`): `apply_delta` (live), `render_for_prompt` (инъекция в системный промпт), `ensure`, `set_mood` (текущий вектор/лицо в портрет), `baseline` (prior настроения из `mood_baseline`, формат `"v,a,d"` с back-compat для старого `"v,a"`). Поля `mood`/`bot_mood`/`mood_baseline` пишет код/weekly, не live-дельта LLM.
...
- **About / портрет** (`about_user.md` + `_user_deltas.jsonl`): см. `bot/about.py` — досье человека (14 секций, описательно от 3-го лица), инъецируется в системный промпт; live-дельты копятся, проза переписывается weekly. Прозаические live-ключи: `speech_note/trigger/motif/fact/rapport/style/passion/letdown/epistemics/attachment/routine`.
...
- **Портрет носителя** `about_user.md` (`bot/about.py`): live-дельты в frontmatter + журнал, инъекция в системный промпт; прозу 14 секций пишет weekly. `/about` показывает портрет словами.
