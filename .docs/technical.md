...
- **About / портрет** (`about_user.md` + `_user_deltas.jsonl`): см. `bot/about.py` — досье человека (11 секций, описательно от 3-го лица), инъецируется в системный промпт; live-дельты копятся, проза переписывается weekly. Прозаические live-ключи: `speech_note/trigger/motif/fact/rapport/style/passion/letdown`.
...
- **Портрет носителя** `about_user.md` (`bot/about.py`): live-дельты в frontmatter + журнал, инъекция в системный промпт; прозу 11 секций пишет weekly. `/about` показывает портрет словами.
...
- **2026-05-22:** портрет расширен с 8 до 11 секций — добавлены «Стиль», «Страсти (что вдохновляет)», «Огорчает / разочаровывает» (`bot/about.py::_SECTIONS`). Появились live-ключи `user_delta`: `style/passion/letdown` (`_PROSE_KEYS` + контракт `prompts/process.md`); копятся в `_user_deltas.jsonl`, прозу синтезирует weekly-review (обновлён список секций и ключей в SKILL.md). Frontmatter не менялся.
