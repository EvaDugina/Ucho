...
- **Self-service reset:** `/leta` удаляет только `<vault>/users/<current_uid>/` после точной фразы `/leta УДАЛИТЬ <uid>`. Перед `shutil.rmtree` сервис сверяет resolved path с ожидаемым `VAULT_PATH/users/<uid>`, пишет scoped git safety commit, очищает runtime-сессию и логирует только uid. После успешного удаления transport best-effort чистит Telegram-чат по известным/recent `message_id`; старые сообщения могут остаться из-за ограничений Bot API. `.psycho/`, `.git/`, whitelist/consent, чужие `users/<other_uid>/` и git history не чистятся.
- **Потоки данных:**
...
- `bot/services/deletion_service.py` — опасная операция `/leta`: проверяет request-scoped uid и resolved path, удаляет только `users/<uid>/` через `git_wrap("delete user data")`, затем забывает runtime-сессию; также собирает известные `message_id` для best-effort Telegram-cleanup; не трогает `.psycho/`, `.git/` и allow-list.
