...
- **Self-service reset:** `/leta` очищает содержимое `<vault>/users/<current_uid>/` после точной фразы `/leta УДАЛИТЬ <uid>`. После подтверждения transport сначала отправляет грубую стартовую фразу смытия, затем сервис сверяет resolved path с ожидаемым `VAULT_PATH/users/<uid>`, пишет scoped git safety commit, очищает runtime-сессию, заново создаёт пустой layout как после первого `/start` и логирует только uid. После успешного reset transport best-effort растворяет Telegram-чат по известным/recent `message_id` по одному сообщению с небольшой задержкой; старые сообщения могут остаться из-за ограничений Bot API. При ошибке reset сообщение об ошибке остаётся в чате, chat purge не запускается. Корневая папка пользователя, `.psycho/`, `.git/`, whitelist/consent, чужие `users/<other_uid>/` и git history не чистятся.
- **Потоки данных:**
...
- **2026-06-10:** `/leta` получила грубую стартовую фразу смытия и постепенное растворение Telegram-чата: transport удаляет известные/recent `message_id` по одному с небольшой задержкой, добавляя в очередь удаления подтверждение, стартовый статус и финальное `Смыто.`. Ошибка reset оставляет диагностическое сообщение в чате и не запускает chat purge. `/pebble` теперь проходит через busy-middleware и отвечает «Больно.» даже во время LLM-генерации.
