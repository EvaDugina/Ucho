...
Приложение собирает YAML-блок, само добавляет московскую дату и число уникальных
raw_event_id учтённых дельт. Telegram timestamp перед сохранением переводится
в `DAILY_TZ`. Внешние Markdown-ограждения снимаются при чтении, записи и показе.
Профиль без шести обязательных полей однократно дополняется при `/about`:
