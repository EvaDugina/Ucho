...
- `bot/llm.py` — AITunnel-обёртка openai-клиента с task-aware routing/fallback. Режимы `ask` / `process` + `about_present` (портрет; `iuda.md` + `about.md`) + `classify_mood` (категории настроения `sign/energy/direction/quality/dominance`; принимает лексиконный `vad`-якорь) + `analyze_psych` (OCEAN/PANAS для owner-анализа) + `regenerate_reaction` (только новая реплика в выбранном лице, без записи графа) + `pebble_reply` (короткая fast-реплика на `/pebble`). JSON-вызовы идут с `response_format={"type":"json_object"}`; provider-specific `extra_body.provider` больше не отправляется. Primary по умолчанию: `qwen3-235b-a22b-2507`; compose/example fallback и fast-маршрут — `deepseek-v4-flash`; task-specific env может переопределить маршрут. Если все модели маршрута недоступны, бот логирует сбой и молчит пользователю; исключение — `/pebble`, где fallback = случайная сгенерированная фраза из `moods.PEBBLE_FALLBACK_REPLIES`. Системный промпт = `iuda` + `base` + `mood.md` + addendum + `user_prompt` + портрет.
...
- **Healthcheck/stone:** `/pebble` команда в Telegram — короткая fast-реплика Иуды через дешёвый маршрут AITunnel; при LLM-сбое отвечает случайной заготовленной репликой из `moods.PEBBLE_FALLBACK_REPLIES`; команда не закрывает сессию. Внешнего healthcheck-endpoint нет.
...
- **Никаких stacktrace пользователю** — все exceptions перехвачены; LLM-сбои молчат наружу, unexpected-сбои получают только нейтральный fallback без traceback.
...
- **Обработка ошибок:** доменные LLM-сбои (`LLMError`: модель недоступна/ответ не разобран) не выводятся в чат: бот молчит и оставляет внутренний лог; исключение — `/pebble`, где fallback берётся случайно из `moods.PEBBLE_FALLBACK_REPLIES`. Прочие unexpected exceptions перехватывает глобальный error-handler; stacktrace пишется в stderr контейнера и `.psycho/log.md`, traceback наружу не уходит.
