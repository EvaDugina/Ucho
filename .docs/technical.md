...
| `ANALYSIS_ENABLED` | мульти-методный разбор ответа для OWNER (сравнение методов) перед основным ответом + `_analysis_log.jsonl`; `false` → только базовый разбор настроения | `true` |
...
- `bot/analysis.py` — оркестратор мульти-методного сравнения (OWNER-тестирование): `run_all` гоняет провайдеры конкурентно (переиспользует `mood_vec`/`vad` из пайплайна настроения + `emolex` + `dostoevsky` + `llm.analyze_psych`), `format_report` собирает единое сообщение (шлётся перед ответом), `log_analysis` пишет выводы всех методов в `_analysis_log.jsonl` (кольцо 200) для сравнения точности. Гейт — OWNER + `ANALYSIS_ENABLED`.
