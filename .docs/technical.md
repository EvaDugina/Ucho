...
Для `ask`, `process` и `about_present` системное сообщение собирается из
`prompts/base.md`, полного образа Иуды в `prompts/judas.md` и режима
`ask.md`/`process.md`/`about.md`. Собеседник не получает литературного титула; сюжет
персонажа не объявляется общим прошлым с ним. На просьбу задать вопрос
`process.md` разрешает один вопрос в реакции, на поправку — признание ошибки.
Короткий неоднозначный текст не служит основанием для догадок о человеке.
`classify_mood` и `synthesize_about` используют отдельные нейтральные системные
правила; personality-дельты в `process` остаются выводами от третьего лица.
...
- `process`: reaction голосом Иуды + нейтральные personality deltas.
