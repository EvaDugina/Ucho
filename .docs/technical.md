...
экземпляр Telegram Bot отправляется только `OWNER_TELEGRAM_ID` и подавляется
до следующего успешного LLM-ответа. Состояние подавления хранится в памяти
процесса; ошибка отправки Telegram не заменяет исходную ошибку LLM.
