...
- `bot/analysis.py` — оркестратор мульти-методного сравнения (OWNER-тестирование): `run_all` гоняет провайдеры конкурентно (переиспользует `mood_vec`/`vad` из пайплайна настроения + `emolex` + `dostoevsky` + `llm.analyze_psych`), `format_report` собирает единое сообщение с текстовой расшифровкой каждого числа (шлётся перед ответом), `append_point` пишет точку в **durable** помесячный ряд `mood/timeseries/YYYY-MM.jsonl` (append-only, без ротации — основа для графиков день/неделя/месяц/сезон/год), `rebuild_chart` перегенерирует заметку `mood/График настроения.md` с блоком Obsidian Charts (дневное среднее PAD; рисует community-плагин, Python не нужен), `aggregate_daily` — чистая агрегация. Гейт — OWNER + `ANALYSIS_ENABLED`.
