...
- `bot/mood_file.py` — живой черновик настроения `personality/mood.md` (capture-first): `set_current` (код пишет снимок из `moods`: эмоция/V/A/D/устойчивость/лицо), `baseline` (prior `mood_baseline` "v,a,d", back-compat "v,a"; пишет weekly-review), `render_for_prompt` (короткая строка настроения в промпт персоны), `ensure` (миграция mood-полей из legacy about_user.md). Выверенный граф настроений — в `mood/` (weekly).
...
- **Портрет носителя** `personality/about.md` (`bot/about.py`): live-дельты в frontmatter + журнал, инъекция в системный промпт; прозу 14 секций пишет weekly. `/about` показывает портрет словами. Настроение — в `personality/mood.md` (`bot/mood_file.py`), пишется кодом каждый ход.
