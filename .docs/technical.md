...
- **Разделение труда (capture-first):** live-модель выбранного provider только захватывает — диалог, `00_raw/`, **черновые** worldview-атомы `status: draft` в областях `01-04` и слабый `mask_frequency_draft`. Она может предложить мягкие `related/influences`, но stable-связи и реальные противоречия утверждает сильная модель. Выверенные документы строятся вручную двумя скиллами (proposal → apply под git): `.agents/skills/reconcista/` — граф мировоззрения (промоушн `draft → stable`, дедуп/слияние, связи внутри и между областями, реальные противоречия, MOC); `.agents/skills/depersonalization/` — `05_Общее/about.md`, `05_Общее/summary.md`, `05_Общее/profile.md`, `05_Общее/softskills.md`, curated `05_Общее/mask_frequencies.json`, `05_Общее/user_prompt.md` и нарратив настроения в `01_Мироощущение/mood/mood.md`.
- **Multi-user изоляция:** бот обслуживает несколько доверенных пользователей; у каждого — своя база в `<vault>/users/<user_id>/` (`00_raw`, `01_Мироощущение`, `02_Миропонимание`, `03_Ценностно-нормативная подсистема`, `04_Практический уровень`, `05_Общее`, `_index`, `_state`, `_session`). `.psycho/` (manifest, log, startup-check, users.json) и `.git/` — глобальные на корне (один safety net, ключи манифеста — относительные пути). Текущий пользователь — request-scoped через `userctx` (contextvar, async-безопасно): aiogram-middleware ставит его на каждый update; data-слой маршрутизирует пути по `userctx.user_root()`. Whitelist + роли — `bot/users.py` (`OWNER_TELEGRAM_ID` = админ, гости — env + `.psycho/users.json`). Гостю при первом обращении — disclaimer о приватности.
- **Self-service reset:** `/leta` удаляет только `<vault>/users/<current_uid>/` после точной фразы `/leta УДАЛИТЬ <uid>`. Перед `shutil.rmtree` сервис сверяет resolved path с ожидаемым `VAULT_PATH/users/<uid>`, пишет scoped git safety commit, очищает runtime-сессию и логирует только uid. `.psycho/`, `.git/`, whitelist/consent, чужие `users/<other_uid>/` и git history не чистятся.
- **Потоки данных:**
  - Пользователь шлёт сообщение → `00_raw/sessions` фиксирует событие до LLM → `01_Мироощущение/mood` анализирует тон → LLM возвращает JSON (`worldview_observations` — только анализ) → код пишет в vault: `00_raw/qna`, `01-04/atoms`, `05_Общее/deltas`; slug из имени, дедуп по всему worldview-графу решает create/update → атомарная запись через `git_wrap` → area-MOC rebuild.
  - Если во время генерации пользователь присылает ещё текст или `/echo <текст>`, transport не зовёт LLM второй раз сразу: текст склеивается в `queued_answer` через пустую строку, привязывается к snapshot старого вопроса и после текущего ответа обрабатывается одним следующим LLM-запросом. `/start` удаляет только этот merge-slot; текущую генерацию не прерывает. Другие команды в busy-состоянии отвечают `Ещё думаю.` и не закрывают сессию.
  ...
  - APScheduler раз в день → `send_daily_question` → handler в обход Telegram-входа. Дедуп по дате (`vault.daily_already_sent`/`mark_daily_sent`, поле `last_daily_date` в `_state.json`) — один дневной на пользователя в день, общий для cron и догона. При старте `scheduler.catch_up_daily` досылает сегодняшний дневной, если бот лежал в час рассылки (за прошлые дни — нет). Активная сессия/прошлые ответы не блокируют дневной. Отдельный evening-reminder cron в `DAILY_REMINDER_START` собирает тех, кто не ответил на сегодняшнее `last_daily_q_num`, выбирает одно batch-время до `DAILY_REMINDER_END`, повторно проверяет ответ перед отправкой и пишет `kind=reminder` в тот же session-log без смены `last_question`.
  - При старте контейнера → `selfcheck.run()` (механический, без LLM): MOC rebuild областей 01-04 + проверка битых path-wikilink, self-link, orphan atoms, дублей summary и симметрии `contradicts` + `.psycho/startup-check.md`. Затем `session.restore_all()` восстанавливает активные `_session.json`, `process_pending_on_startup` дожимает pending-события по `pending_answer_event_id`, затем `process_queued_on_startup` дожимает `queued_answer` и только после этого сливается Telegram offline backlog. Polling не выставляет `drop_pending_updates`.
...
- `bot/config.py` — env-переменные и пути `VAULT_PATH / MANIFEST_PATH / LOG_PATH`.
- `bot/worldview_taxonomy.py` — каноническая таксономия мировоззрения: области 01-04, категории, темы, описания, выбор случайной цели и targeted-маппинг свободного текста.
- `bot/worldview.py` — `WorldviewAtom`, YAML/frontmatter roundtrip, хранение `01-04/atoms`, дедуп по всему графу, path-wikilink связи и area-MOC.
- `bot/handlers.py` — Telegram transport: routers, команды, callback parsing, Telegram send/reply. Сценарии conversation/note/daily/delete вынесены в `bot/services/*`; `handlers.py` больше не держит основную бизнес-логику записи графа. Команды (кроме `/pebble`, `/regen`, `/like`, `/remask`, `/leta`) закрывают активную сессию в `AccessMiddleware`; `ask/echo/ucho/about` открывают новую. В busy-состоянии `/echo` добавляет текст в `queued_answer`, `/start` удаляет queue без прерывания текущей генерации, остальные команды отвечают `Ещё думаю.`. Reply: `on_text` сперва ищет session по `00_raw/sessions`, иначе реконструирует вопрос из тела reply/сохраняет заметку.
...
- `bot/services/deletion_service.py` — опасная операция `/leta`: проверяет request-scoped uid и resolved path, удаляет только `users/<uid>/` через `git_wrap("delete user data")`, затем забывает runtime-сессию; не трогает `.psycho/`, `.git/` и allow-list.
...
- `bot/services/answer_service.py` — запись `worldview_observations` в граф: `00_raw/qna` дословно + `area/category/theme` сессии → slug=`slugify(name)`, дедуп `worldview.resolve_slug`/Jaccard по всем областям → draft/append evidence → area-MOC; плюс `about.apply_delta`. Stable-связи/конфликты утверждает reconcista.
...
- `bot/analysis.py` — оркестратор инструментального анализа (OWNER-тестирование): `run_all` гоняет провайдеры конкурентно (переиспользует `mood_vec` из пайплайна настроения + `emolex` + `dostoevsky`, затем считает code-derived PANAS), `format_report` пишет в `01_Мироощущение/mood/analysis/YYYY-MM-DD.md` только итоговые поля: **PAD: эмоция + выбранное лицо Иуды**, **NRC-EmoLex: ведущие эмоции + полярность**, **Dostoevsky**, **PANAS**; в Telegram отчёт не отправляется. `append_point` пишет точку в durable помесячный ряд `01_Мироощущение/mood/timeseries/YYYY-MM.jsonl`, `rebuild_chart` перегенерирует `01_Мироощущение/mood/График настроения.md`. Big Five/OCEAN не считается в live-анализе: его пишет depersonalization в `05_Общее/profile.md` по корпусу `00_raw/`.
- `bot/sensation_analysis/` — отдельный пакет анализа полного канона `01_Мироощущение`: `taxonomy.py` держит 6 категорий и 103 темы из `knowledge.md`, `signals.py` собирает лёгкие подсказки PAD/VAD/EmoLex/Dostoevsky/маркеры, `prompt.py` строит compact prompt, `validation.py` отбрасывает неизвестные темы, низкую уверенность и недословные quote, `analyzer.py` вызывает только API-модель AITunnel/OpenRouter через `llm.analyze_sensation_json`, пишет диагностический отчёт в `01_Мироощущение/analysis01/` и мерджит валидные кандидаты в обычные `worldview_observations` перед `answer_service.apply_processed`.
- `bot/understanding_analysis/` — отдельный пакет анализа полного канона `02_Миропонимание`: `taxonomy.py` держит 6 категорий и 82 темы из `knowledge.md`, `signals.py` собирает лёгкие маркеры источников знания, убеждений, принципов, причинности, self-model и неизвестности, `prompt.py` строит compact prompt, `validation.py` отбрасывает неизвестные темы, низкую уверенность и недословные quote, `analyzer.py` вызывает только API-модель AITunnel/OpenRouter через `llm.analyze_understanding_json`, пишет диагностический отчёт в `02_Миропонимание/analysis02/` и мерджит валидные кандидаты в обычные `worldview_observations` перед `answer_service.apply_processed`.
- `bot/values_norms_analysis/` — отдельный пакет анализа полного канона `03_Ценностно-нормативная подсистема`: `taxonomy.py` держит 6 категорий и 88 тем из `knowledge.md`, `signals.py` собирает лёгкие маркеры ценностей, идеалов, норм, табу, иерархий и моральной оценки, `prompt.py` строит compact prompt, `validation.py` отбрасывает неизвестные темы, низкую уверенность и недословные quote, тип атома выводит из категории, `analyzer.py` вызывает только API-модель AITunnel/OpenRouter через `llm.analyze_values_norms_json`, пишет диагностический отчёт в `03_Ценностно-нормативная подсистема/analysis03/` и мерджит валидные кандидаты в обычные `worldview_observations` перед `answer_service.apply_processed`.
- `bot/practice_analysis/` — отдельный пакет анализа полного канона `04_Практический уровень`: `taxonomy.py` держит 6 категорий и 95 тем из `knowledge.md`, `signals.py` собирает лёгкие маркеры готовности, воли, стиля жизни, поступков, стратегий совладания и последствий выбора, `prompt.py` строит compact prompt, `validation.py` отбрасывает неизвестные темы, низкую уверенность и недословные quote, тип атома выводит из категории, `analyzer.py` вызывает только API-модель AITunnel/OpenRouter через `llm.analyze_practice_json`, пишет диагностический отчёт в `04_Практический уровень/analysis04/` и мерджит валидные кандидаты в обычные `worldview_observations` перед `answer_service.apply_processed`.
...
- `bot/moc.py` — legacy-совместимость старого графа; целевой runtime использует `worldview.rebuild_area_moc(area)`.
...
- `prompts/base.md` (механика, таксономия мировоззрения, формат) + `prompts/iuda.md` (персона и голос) + `prompts/ask.md` / `process.md` / `about.md` / `mood.md` / `questions_examples.md` — промпты по режимам. `llm._system(kind)` собирает общий слой + addendum режима + портрет; JSON-контракт строгий. (`review.md`, `summarize.md`, `seeds.md` удалены.)
- `bot/sessions.py` — восстановимая обёртка поверх `00_raw/sessions`: `snapshot` no-op, `load`/`find_by_message_id` для reply-resume.
- `bot/session_log.py` — машинный append-only журнал всех сообщений активной сессии в `00_raw/sessions/<session_id>.jsonl`: user/assistant, даты Telegram, kind, message_id, q_num, `area/category/theme`, legacy `domain`, bot_mood.
- `bot/face_actions.py` — per-user action records для reply-действий над репликами Иуды: `05_Общее/face_actions.json`, оценки `01_Мироощущение/mood/feedback.jsonl`, избранное `05_Общее/liked_replies.json` и `05_Общее/liked_replies_log.jsonl`.
- `bot/about.py` — портрет носителя (`05_Общее/about.md` + `05_Общее/deltas.jsonl`): `apply_delta` (live машинные поля + журнал), `render_for_prompt` (инъекция в системный промпт), `render_about_context` для `/about` (about + mood summary/narrative из `01_Мироощущение/mood/mood.md` + `05_Общее/profile.md`), `ensure` (создаёт пустой скелет).
- `bot/mood_file.py` — живой черновик настроения `01_Мироощущение/mood/mood.md` (capture-first): `set_current` (код пишет снимок из `moods`: эмоция/V/A/D/устойчивость/лицо), `baseline` (prior `mood_baseline` "v,a,d", back-compat "v,a"; пишет depersonalization), `render_for_prompt` (короткая строка настроения в промпт персоны), `ensure` (пустой скелет). Тело-нарратив анализа настроения пишет depersonalization, код тело сохраняет.
- `scripts/migrate_worldview_schema.py` — одноразовый dry-run/apply перенос старого vault: `02_concepts` → `01-04/atoms`, `03_personality` → `05_Общее`, `01_mood` → `01_Мироощущение/mood`; `00_raw` не меняет.
...
- **WorldviewAtom** (`<01-04 область>/atoms/<slug>.md`): frontmatter `area/category/theme/type/slug/created/updated/status/aliases/evidence/supports/contradicts/derived_from/related/influences/manifests_as/confidence/source_session`, тело — callouts `[!summary]`, `[!quote]`, `[!question]`, `[!source]`. `status`: `draft` (создан ботом live, ascii-slug) → `stable` (выверен сильной моделью). Связи хранятся path-wikilink’ами вида `[[02_Миропонимание/atoms/svoboda|Свобода]]`; `contradicts` симметрична, остальные направленные.
- **Raw-блок** (`00_raw/qna/YYYY-MM-DD.md`): `## Q<N> · HH:MM · <area>/<category>/<theme>`, `**Q:** …`, `**A:** …`, `^Q<N>` block-id на отдельной строке. Старые raw с `<domain>` читаются через legacy-маппинг, но новые записи `domain` не используют.
- **Session raw-log** (`00_raw/sessions/<session_id>.jsonl`): машинный append-only журнал активной сессии, по одной строке на сообщение: `{ts, session_id, role, kind, message_id, reply_to_message_id, q_num, area, category, theme, theme_key, bot_mood, text}` + legacy `domain` только если оно уже было в старом событии. `kind=reminder` — вечернее напоминание по daily Q; оно хранит тот же `q_num`, но не становится новым `last_question`. `ts` берётся из Telegram `message.date`/`sent.date`, fallback — текущее время; порядок строк — порядок обработки событий.
...
- **Face actions / feedback / likes**: `05_Общее/face_actions.json` хранит короткоживущие action-token записи с `session_id`/`event_id` refs, лицом, `root_token` цепочки регенераций и message_id, без копий полного user/assistant текста. `01_Мироощущение/mood/feedback.jsonl` — append-only оценки ответов: `1` за избранное; перегенерация больше не считается отрицательной оценкой. `05_Общее/liked_replies.json` — текущее состояние избранных ответов по token/message_id, `05_Общее/liked_replies_log.jsonl` — история добавлений; хранят refs на session events. Первый `/like` по ответу дополнительно повышает draft-частоту его маски. Вопросы не имеют rateable action и не пишут feedback.
- **Mask frequencies:** `05_Общее/mask_frequencies.json` — curated per-user JSON `{mask: coefficient}` с коэффициентами 0..1 для автоматического выбора масок; файл заполняет и редактирует только depersonalization. Бот читает его read-only и пишет только `05_Общее/mask_frequencies_draft.json`: live-черновик из `process.mask_frequency_draft` после каждого ответа + like-derived коэффициенты. Effective-частота = максимум curated и draft.
- **Разрешённые runtime-файлы бота:** per-user `00_raw/sessions/*.jsonl`, `00_raw/qna/*.md`, `00_raw/notes/*.md`, `<01-04 область>/atoms/*.md`, `<01-04 область>/MOC.md`, `01_Мироощущение/analysis01/*.md`, `02_Миропонимание/analysis02/*.md`, `03_Ценностно-нормативная подсистема/analysis03/*.md`, `04_Практический уровень/analysis04/*.md`, `01_Мироощущение/mood/events/*.jsonl`, `01_Мироощущение/mood/analysis/*.md`, `01_Мироощущение/mood/timeseries/*.jsonl`, `01_Мироощущение/mood/График настроения.md`, `01_Мироощущение/mood/feedback.jsonl`, `05_Общее/about.md`, `05_Общее/summary.md`, `05_Общее/profile.md`, `05_Общее/softskills.md`, `05_Общее/deltas.jsonl`, `05_Общее/mask_frequencies_draft.json`, `05_Общее/face_actions.json`, `05_Общее/liked_replies.json`, `05_Общее/liked_replies_log.jsonl`, `_state.json`, `_session.json`, `_index.md`, `.obsidian/graph.json`; global `.psycho/manifest.json`, `.psycho/log.md`, `.psycho/startup-check.md`, `.psycho/users.json`. Подтверждённая `/leta` может удалить всё per-user поддерево `users/<uid>/`; глобальные файлы остаются.
...
- **05_Общее** (`05_Общее/about.md` + `05_Общее/summary.md` + `05_Общее/profile.md` + `05_Общее/softskills.md`): краткая сводка о пользователе, бывший personality-контур и сжатые выводы по областям 01-04. `about.py` ведёт live-дельты в `05_Общее/deltas.jsonl`, прозу и общий `summary.md` пишет depersonalization; настроение хранится отдельно в `01_Мироощущение/mood/mood.md`. `/about` использует about + mood narrative + profile.
- **Контракт LLM `process`-режима (LLM только анализ + реакция — запись в БД делает код):**
  ```json
  {
    "type": "processed",
    "worldview_observations": [{"area": "02_Миропонимание", "category": "principles", "theme": "держать слово", "type": "principle", "name": "Нарушение слова", "summary": "...", "quote": "дословный фрагмент ответа", "confidence": 0.78}],
  ...
  LLM **не присылает** `slug`, `raw_entry`, `concepts_to_create/update`, stable-связи — код их игнорирует. Идентичность/запись на коде: slug выводит `validation.slugify` из `name`, `area/category/theme` берутся из сессии или observation, create-vs-update решает worldview-дедуп (`resolve_slug` по имени/slug/aliases + Jaccard), `quote` валидируется как дословная подстрока ответа. Граф stable-связей и конфликтов строит reconcista.
...
│   ├── worldview_taxonomy.py канон 01-04: области, категории, темы, random/targeted
│   ├── worldview.py        WorldviewAtom + рендер/парсер + dedup/resolve + area-MOC
│   ├── graph.py            legacy Concept dataclass для чтения/миграций старого vault
...
│   ├── questions.py        совместимость: последние вопросы из 00_raw/sessions
│   ├── about.py            портрет носителя (05_Общее/about.md + дельты)
│   ├── mood_file.py        живой черновик настроения (01_Мироощущение/mood/mood.md)
...
│   ├── sensation_analysis/ полный API-only анализ 01_Мироощущение
│   ├── understanding_analysis/ полный API-only анализ 02_Миропонимание
│   ├── values_norms_analysis/ полный API-only анализ 03_Ценностно-нормативная подсистема
│   ├── practice_analysis/ полный API-only анализ 04_Практический уровень
...
│   ├── moc.py              legacy per-domain MOC rebuild
...
В самом vault при первом запуске создаются: `.git/`, `.gitignore`, `.psycho/manifest.json`, `.psycho/log.md`, `00_raw/sessions/`, `00_raw/qna/`, `00_raw/notes/`, четыре области мировоззрения `01_Мироощущение/`, `02_Миропонимание/`, `03_Ценностно-нормативная подсистема/`, `04_Практический уровень/` с `atoms/` и `MOC.md`, `01_Мироощущение/mood/`, `05_Общее/` (`about.md`, `summary.md`, дельты и служебные файлы), `_index.md`, `_state.json`. `05_Общее/profile.md` и `05_Общее/softskills.md` создаёт скилл depersonalization. При каждом старте — `.psycho/startup-check.md`.
...
- **Целевое покрытие:** не отслеживается (PoC B). Главное — happy path случайной/targeted генерации `area/category/theme`, запись атомов 01-04, recovery/queue и drift-сценарий.
...
- **Ручные:** 
  - `/ask` без аргумента → выбирается валидная область/категория/тема, вопрос приходит с этой тройкой.
  - `/ask <свободная тема>` → targeted-маппинг выбирает ближайшую категорию/тему или одноразовый `custom_theme`.
...
- **Whitelist callback `ask:<area>`** — только `any` или ключ одной из четырёх областей 01-04.
- **LLM возвращает только то что в whitelist:** area/category/theme/type/status/relation kind проверяются по closed-list, фолбэки логируются.
...
- Ручная правка worldview-атома в Obsidian → следующий ответ не перетёр правку (drift detection сработал).
- Open Graph View в Obsidian с фильтром по `*/atoms` показывает узлы 01-04 и связи внутри/между областями.
...
- **Диалог:** главный вопрос (`/ask`/`/echo`/дневной) открывает сессию-обсуждение; на каждый ответ — реакция-укол от 1-го лица (НЕ вопрос), сессия открыта, пока не задан новый вопрос или не выполнена любая команда (`/pebble`, `/regen`, `/like`, `/remask`, предупреждение `/leta` — исключения, не трогают сессию). Уточняющих вопросов бот не задаёт. Reply на любую реплику из `00_raw/sessions` её продолжает (`bot/sessions.py`, `session.resume`).
- **Голос:** в чат — от первого лица, на «ты», себя не называет. Досье (`05_Общее/summary.md`, `05_Общее/about.md`) — описательно от 3-го лица. Промпты разнесены: `prompts/iuda.md` (персона) + `base.md` (таксономия мировоззрения и JSON-контракт) + аддендумы `ask`/`process`.
- **Capture-first:** live-модель только захватывает — `process` отдаёт `worldview_observations` + `reaction` + `user_delta` + слабый `mask_frequency_draft`; запись/идентичность (raw дословно, slug из имени, дедуп, верификация цитаты) на коде. Связи внутри/между 01-04, реальные противоречия, промоушн `draft→stable` и общий summary — Codex вручную (`reconcista`/`depersonalization`).
- **Портрет носителя** `05_Общее/about.md` (`bot/about.py`): live-дельты в frontmatter + журнал, инъекция в системный промпт; прозу 20 секций пишет depersonalization. `/about` показывает портрет словами. Настроение — в `01_Мироощущение/mood/mood.md` (`bot/mood_file.py`). `05_Общее/profile.md` и `05_Общее/softskills.md` — выверенные документы depersonalization.
- **Команды:** `/ask /echo /ucho /about /pebble /regen /like /remask /leta /start /help` + админ (`/adduser`/`/removeuser`/`/users`). `/start` закрывает сессию и удаляет ещё не обработанный `queued_answer`; уже начатый LLM-вызов не прерывает. `/leta` без аргументов показывает предупреждение, подтверждение — точная фраза `/leta УДАЛИТЬ <uid>`. Индикатор «Думаю» — один стикер 🎰, только для `/ask` и `/about`; busy-ответ без стикера — `Ещё думаю.`.
...
- **Drift на ручную правку:**
  - Предусловия: атом `<vault>/01_Мироощущение/atoms/trevoga.md` или любой другой `<01-04>/atoms/<slug>.md` существует.
  - Шаги: открой в Obsidian, добавь строку, сохрани. В Telegram задай вопрос про честность, ответь. Бот пишет ответ.
  - Ожидаемый результат: твоя правка на месте, в `.psycho/log.md` появилась строка `drift_skipped` или операция прошла на другой файл; атом не перезаписан целиком.

- **Черновик без связей (capture-first):**
  - Шаги: задай вопрос, ответь развёрнуто. Бот создаёт worldview-атом.
  - Ожидаемый результат: новый файл `<01-04 область>/atoms/<slug>.md` со `status: draft`, `area/category/theme`, evidence и без stable-противоречий. Выверенные связи появятся только после прогона `reconcista`.
...
- **MOC автообновление:**
  - Предусловия: `<vault>/02_Миропонимание/atoms/` пустой или не существует.
  - Шаги: задай вопрос из области `02_Миропонимание`, ответь, дай LLM создать атом.
  - Ожидаемый результат: появился или обновился `02_Миропонимание/MOC.md` с разделом нужной категории и пунктом — новым черновым атомом.

- **reconcista строит граф мировоззрения (Claude/Codex):**
  - Предусловия: несколько `draft`-атомов за неделю.
  - Шаги: в Claude Code запусти скилл `reconcista`, подтверди план (Фаза 2). Портрет/настроение — скилл `depersonalization`.
  - Ожидаемый результат: черновики стали `stable` (или слиты), появились связи внутри и между 01-04, реальные противоречия, `05_Общее/summary.md` получил краткие выводы по областям; `git log` vault содержит пару `reconcista … before/applied`; `00_raw/` не тронут.
...
- **2026-06-09:** добавлена `/leta` для self-service удаления рабочей базы текущего пользователя. Реализация изолирована в `deletion_service`: resolved path должен быть ровно `VAULT_PATH/users/<uid>`, удаление идёт через scoped `git_wrap`, а `_git_commit` стадит pathspec через `git add -A -- users/<uid>`, чтобы deletion commit видел удалённые файлы. `/start` остаётся смывом сессии и дополнительно удаляет ещё не обработанный `queued_answer`.
...
- **2026-06-01:** добавлен provider-aware live-LLM: `OPENROUTER_API_KEY` включает OpenRouter с приоритетом, AITunnel остаётся fallback без изменения capture-first контракта. Комментарии Иуды теперь чистятся отдельным sanitizer и теряют запятые; вопросы и `/about` пунктуацию не режут. `/about` получает context из `about.md`, `mood.md` и `profile.md`. Для сообщений во время генерации добавлен durable merge-slot `queued_answer`; теперь его удаляет `/start`, а остальные команды в busy-состоянии отвечают `Ещё думаю.`.
...
- **2026-06-09:** добавлен API-only анализ полного канона `01_Мироощущение` в отдельном пакете `bot/sensation_analysis/`. Для OWNER при `ANALYSIS_ENABLED=true` бот после обычного `process` вызывает `llm.analyze_sensation_json` через AITunnel/OpenRouter, валидирует category/theme/quote/confidence, пишет диагностику в `01_Мироощущение/analysis01/` и добавляет валидные draft-кандидаты в обычный `answer_service.apply_processed`. Локальные transformer/embedding-модели не добавлялись.
- **2026-06-09:** добавлен API-only анализ полного канона `02_Миропонимание` в отдельном пакете `bot/understanding_analysis/`. Для OWNER при `ANALYSIS_ENABLED=true` бот после обычного `process` вызывает `llm.analyze_understanding_json` через AITunnel/OpenRouter, валидирует category/theme/quote/confidence, пишет диагностику в `02_Миропонимание/analysis02/` и добавляет валидные draft-кандидаты в обычный `answer_service.apply_processed`. Локальные transformer/embedding-модели не добавлялись.
- **2026-06-09:** добавлен API-only анализ полного канона `03_Ценностно-нормативная подсистема` в отдельном пакете `bot/values_norms_analysis/`. Для OWNER при `ANALYSIS_ENABLED=true` бот после обычного `process` вызывает `llm.analyze_values_norms_json` через AITunnel/OpenRouter, валидирует category/theme/quote/confidence, выводит type из категории, пишет диагностику в `03_Ценностно-нормативная подсистема/analysis03/` и добавляет валидные draft-кандидаты в обычный `answer_service.apply_processed`. Локальные transformer/embedding-модели не добавлялись.
- **2026-06-09:** добавлен API-only анализ полного канона `04_Практический уровень` в отдельном пакете `bot/practice_analysis/`. Для OWNER при `ANALYSIS_ENABLED=true` бот после обычного `process` вызывает `llm.analyze_practice_json` через AITunnel/OpenRouter, валидирует category/theme/quote/confidence, выводит type из категории, пишет диагностику в `04_Практический уровень/analysis04/` и добавляет валидные draft-кандидаты в обычный `answer_service.apply_processed`. Локальные transformer/embedding/action-classifier модели не добавлялись.
...
- **Разделение труда (две модели, разные роли):**
  - **Live-LLM provider** — захват: режимы `ask`, `process` (capture-first: только черновики + evidence, без связей/конфликтов), `classify_mood`, `analyze_sensation_json` (owner-only draft-кандидаты 01), `analyze_understanding_json` (owner-only draft-кандидаты 02), `analyze_values_norms_json` (owner-only draft-кандидаты 03), `analyze_practice_json` (owner-only draft-кандидаты 04), `analyze_psych`, `regenerate_reaction` и `about_present` (портрет). OpenRouter включается при непустом `OPENROUTER_API_KEY`; AITunnel остаётся fallback.
