...
Приложение собирает YAML-блок с пятью русскими названиями и числом уникальных
raw_event_id учтённых дельт. `updated` в блок не входит; `null` и low/medium/high
...
Профиль без пяти обязательных полей однократно дополняется при `/about`:
