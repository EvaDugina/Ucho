...
Synthesis возвращает JSON с текстом и семью обязательными характеристиками;
...
Приложение собирает YAML-блок только из семи разрешённых русских полей;
`null` и low/medium/high показываются по-русски.
...
В начале профиля, перед прежними характеристиками, находятся три предпочтения:

| Поле | Точное значение | Допустимые значения |
|---|---|---|
| **Предпочтительная подробность ответов** (`preferred_response_detail`) | Желаемый объём и глубина ответа собеседника: сколько объяснений, аргументов, примеров и шагов нужно. | `brief` — кратко, суть в нескольких фразах; `balanced` — умеренно подробно, основная мысль с пояснением; `detailed` — подробно, с развёрнутыми объяснениями; `context_dependent` — явно зависит от ситуации. |
...
| **Предпочтительный темп диалога** (`preferred_dialogue_pace`) | Желаемая скорость продвижения между вопросами и темами, место для обдумывания и развития текущей мысли. | `reflective` — вдумчивый, один шаг за раз; `balanced` — умеренный, закончить мысль и двигаться дальше; `dynamic` — динамичный, короткие обмены и частая смена вопросов/тем; `context_dependent` — явно зависит от ситуации. |

Основание для этих полей — явная общая просьба или несколько согласующихся
наблюдений о реакции на стиль собеседника. Разовое «здесь ответь кратко» сохраняет
свой контекст. Длина собственных реплик, задержки в Telegram и отсутствие возражений
не доказывают предпочтение. Подробность, прямота и темп оцениваются независимо;
прямота не означает провокацию. При отсутствии оснований — `null`, отображаемый
как «недостаточно данных»; balanced и context_dependent не используются как замена
неизвестному. Условия выбора могут поясняться в разделе «Манера речи».
Явные ожидания от собеседника сохраняются в дельтах `relationships` с цитатой и
исходной областью действия. Эти описательные поля не меняют расписание сообщений.

Остальные поля профиля: **Регистр речи** — лексика и способ выражения; **Тон речи** —
повторяющаяся окраска общения, не настроение момента; **Открытость** — степень
самораскрытия в этой переписке (1 минимум личного, 3 избирательно, 5 подробно о
личном и уязвимом); **Переносимость провокаций** — наблюдаемая реакция на реальные
провокационные вопросы (низкая/средняя/высокая), без таких эпизодов — неизвестно.
