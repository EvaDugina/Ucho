...
- **Self-service reset:** `/leta` очищает содержимое `<vault>/users/<current_uid>/` после точной фразы `/leta УДАЛИТЬ <uid>`. После подтверждения transport сначала отправляет грубую стартовую фразу смытия, затем сервис сверяет resolved path с ожидаемым `VAULT_PATH/users/<uid>`, пишет scoped git safety commit, делает best-effort push через существующий `_git_commit()` path, очищает runtime-сессию, заново создаёт пустой layout как после первого `/start` и логирует только uid. После успешного reset transport best-effort растворяет Telegram-чат по известным/recent `message_id` по одному сообщению с небольшой задержкой, затем отправляет видимое `Кончил.`; старые сообщения могут остаться из-за ограничений Bot API. При ошибке reset сообщение об ошибке остаётся в чате, chat purge не запускается. Корневая папка пользователя, `.psycho/`, `.git/`, whitelist/consent, чужие `users/<other_uid>/` и git history не чистятся.
- **Потоки данных:**
  ...
  - Если во время генерации пользователь присылает ещё текст или `/echo <текст>`, transport не зовёт LLM второй раз сразу: текст склеивается в `queued_answer` через пустую строку, привязывается к snapshot старого вопроса и после текущего ответа обрабатывается одним следующим LLM-запросом. `/cancel` удаляет только этот merge-slot; текущую генерацию не прерывает. `/pebble` отвечает статично даже в busy-состоянии. Другие команды в busy-состоянии отвечают `Ещё думаю.` и не закрывают сессию.
...
- **2026-06-10:** `/leta` получила грубую стартовую фразу смытия и постепенное растворение Telegram-чата: reset идёт через `git_wrap`, vault-коммиты пробуют best-effort push через `_git_commit()`, transport удаляет известные/recent `message_id` по одному с небольшой задержкой и после purge оставляет видимое `Кончил.`. Ошибка reset оставляет диагностическое сообщение в чате и не запускает chat purge. `/pebble` теперь проходит через busy-middleware и отвечает «Больно.» даже во время LLM-генерации.
